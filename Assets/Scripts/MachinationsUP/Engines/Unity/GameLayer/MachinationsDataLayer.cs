    ...
    public class MachinationsDataLayer : IGameLifecycleSubscriber, IGameObjectLifecycleSubscriber
    {
        ...
        public static JSONObject GetInitRequestData (string diagramToken)
        {
            ...
            foreach (DiagramMapping diagramMapping in _sourceElements.Keys)
            {
                ...
                {
                    Debug.Log("Skipping requsting info for already initialized Source Element: " + _sourceElements[diagramMapping]);
                    continue;
                }
        ...
        static public MachinationsDataLayer Instance
        {
            get
            {
                ...
                Debug.Log("MGL created by invocation. Hash is " + _instance.GetHashCode() +
                          " and User Key is ???????????????????!!!!!!!!!");
                return _instance;
            }
        }
